...
let icon = passwordWrapper.querySelector("i")
let password = passwordWrapper.querySelector("#password")
let svg = passwordWrapper.querySelector("#passwordWrapper svg")
...
function show() {
    password.type = "text"
    pathsD.hidded.forEach((path, id) => paths[id].setAttribute("d", path))
}

function hide() {
    pathsD.showed.forEach((path, id) => paths[id].setAttribute("d", path))
}
